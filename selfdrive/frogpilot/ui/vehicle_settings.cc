...
FrogPilotVehiclesPanel::FrogPilotVehiclesPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  std::vector<std::tuple<QString, QString, QString, QString>> vehicleToggles {
    ...
    {"CruiseAutoResume", tr("Cruise Auto Resume"), tr("Enable Auto Resume when Adapted Cruise Control On."), ""},
    {"AutoResumeDistance", tr("Auto Resume Distance"), tr("Cruise Auto Resume activate when lead car below the distance."), ""},
    {"AutoResumeSetSpeed", tr("Auto Resume Set Speed"), tr("Cruise Auto Resume activate when below the set speed."), ""},
  ...
  for (const auto &[param, title, desc, icon] : vehicleToggles) {
    ...
    if (param == "LongitudinalTune") {
      ...
      toggle = new FrogPilotParamValueControl(param, title, desc, icon, 16, 22, std::map<int, QString>(), this, false, " mph", 1);
    } else {
...
void FrogPilotVehiclesPanel::updateMetric() {
  ...
  if (isMetric) {
    ...
    arSetSpeedToggle->updateControl(25, 35, tr(" kph"), 1);
  } else {
    arDistanceToggle->updateControl(0, 50, tr(" feet"), 1);
    arSetSpeedToggle->updateControl(16, 22, tr(" mph"), 1);
  }
