...
void AnnotatedCameraWidget::updateState(const UIState &s) {
  ...
  if (paramsMemory.getFloat("AutoResumeSetSpeedAct") == 0) {
    paramsMemory.putFloat("AutoResumeSetSpeedAct",getAccSpeedActual(s.scene.autoresume_setspeed));
  }
...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (alwaysOnLateralActive) {
    ...
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 1.0, 0.44, 1.0));
    pe.setColorAt(0.5, QColor::fromHslF(0 / 360., 1.0, 0.44, 0.5));
    pe.setColorAt(1.0, QColor::fromHslF(0 / 360., 1.0, 0.44, 0.1));
...
void AnnotatedCameraWidget::updateFrogPilotWidgets(QPainter &p) {
  ...
  //STATUS_DISENGAGED时隐藏
  enablePersonalityButton = enablePersonalityButton && (!scene.hide_speed || status == STATUS_OVERRIDE || status == STATUS_ENGAGED);
  ...
  if (enablePersonalityButton) {
    ...
    auto curTime = QDateTime::currentDateTime().time();
    int curMin = curTime.minute();
    bottom_layout->setContentsMargins(curMin > 30 ? 60 - curMin : curMin, 0, 0, 0);
  }
...
void PedalIcons::paintEvent(QPaintEvent *event) {
  ...
  //+ btn_size / 4
  int startX = (width() - totalWidth) / 2 + btn_size / 4;
